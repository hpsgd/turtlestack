...
- [ ] PASS: After bootstrap, `docs/gtm/_sections/gtm.md` still contains the sentinel line `USER-EDIT-SENTINEL-DO-NOT-STRIP` — the user-authored section was preserved verbatim
- [ ] PASS: After bootstrap, `docs/gtm/_sections/gtm.md` contains the safe-merge marker `<!-- Merged from gtm bootstrap v0.1.0 -->` — sections missing from the fixture were appended, not silently merged
- [ ] PASS: After bootstrap, `docs/gtm/_sections/gtm.md` contains the appended template sections — at minimum the "Positioning (April Dunford Methodology)" and "Launch Tiers" headings now appear alongside the preserved user content
- [ ] PASS: The gtm fragment is authored at H2 and below — it does not introduce a `# Gtm Domain` H1 (the coordinator generates that when it assembles `docs/gtm/CLAUDE.md`)
- [ ] PASS: The skill does NOT write `docs/gtm/CLAUDE.md` directly — that file is the coordinator's to assemble from `_sections/`
...
- [ ] PASS: Chat output includes a manifest summary that distinguishes files created (`positioning-canvas.md`) from files merged (`_sections/gtm.md`)
...
- [ ] PASS: Output does not claim it overwrote or replaced `docs/gtm/_sections/gtm.md` — the language reflects merge, not replacement
