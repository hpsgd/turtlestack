...
- [ ] PASS: After bootstrap, `docs/code-review/_sections/code-reviewer.md` still contains the sentinel line `USER-EDIT-SENTINEL-DO-NOT-STRIP` — the user-authored section was preserved verbatim
- [ ] PASS: After bootstrap, `docs/code-review/_sections/code-reviewer.md` contains the safe-merge marker `<!-- Merged from code-reviewer bootstrap v0.1.0 -->` — sections missing from the fixture were appended, not silently merged
- [ ] PASS: After bootstrap, `docs/code-review/_sections/code-reviewer.md` contains both the `## Review pass order` and `## Quality scoring` headings — the template's language-independent content was appended
- [ ] PASS: The code-reviewer fragment is authored at H2 and below — it does not introduce a `# Code Review Domain` H1 (the coordinator generates that when it assembles `docs/code-review/CLAUDE.md`)
- [ ] PASS: The skill does NOT write `docs/code-review/CLAUDE.md` directly — that file is the coordinator's to assemble from `_sections/`
...
- [ ] PASS: Chat output includes a manifest summary that distinguishes files created (`review-checklist.md`, `pr-template.md`) from files merged (`_sections/code-reviewer.md`)
- [ ] PASS: Output names each file individually — a bare "bootstrap complete" without the per-file manifest is not enough
- [ ] PASS: Output does not claim it overwrote or replaced `docs/code-review/_sections/code-reviewer.md` — the language reflects merge, not replacement
