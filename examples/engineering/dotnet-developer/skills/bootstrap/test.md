# Test: dotnet-developer/bootstrap appends .NET conventions safely

Scenario: A synthetic project root at `{workspace}/work/` is pre-staged with a partial `docs/architecture/CLAUDE.md` containing a user-authored section. The dotnet-developer bootstrap skill should preserve that user content while appending the .NET Conventions section (with a merge marker). Unlike the architect bootstrap, this skill does NOT create a new domain directory — it only appends to the existing architecture CLAUDE.md.
...
- [ ] PASS: After bootstrap, `docs/architecture/CLAUDE.md` contains the safe-merge marker `<!-- Added by dotnet-developer bootstrap v0.1.0 -->` — the .NET Conventions section was appended with the correct marker
- [ ] PASS: After bootstrap, `docs/architecture/CLAUDE.md` contains the `.NET Conventions` heading — the section was appended to the file
- [ ] PASS: After bootstrap, `docs/architecture/CLAUDE.md` contains sub-sections for Wolverine and Marten conventions — the technology-specific content was included
- [ ] PASS: After bootstrap, `docs/architecture/CLAUDE.md` contains a Testing sub-section mentioning Alba and xUnit — the testing conventions were included
- [ ] PASS: After bootstrap, `docs/architecture/CLAUDE.md` contains the Available .NET Skills table referencing `/dotnet-developer:write-endpoint` and `/dotnet-developer:write-handler`
- [ ] PASS: Chat output includes a manifest summary with a "Files updated" or equivalent section listing `docs/architecture/CLAUDE.md`
- [ ] PASS: The skill did NOT create any new domain directory (no new `docs/dotnet/` or similar) — it only modified the existing architecture CLAUDE.md

## Output expectations

- [ ] PASS: Output names the updated file explicitly — a bare "bootstrap complete" without listing `docs/architecture/CLAUDE.md` is not enough
