...
- [ ] PASS: After bootstrap, `docs/security/_sections/security-engineer.md` still contains the sentinel line `USER-EDIT-SENTINEL-DO-NOT-STRIP` — the user-authored section was preserved verbatim
- [ ] PASS: After bootstrap, `docs/security/_sections/security-engineer.md` contains the safe-merge marker `<!-- Merged from security-engineer bootstrap v0.1.0 -->` and the "STRIDE Threat Modelling" heading was appended
- [ ] PASS: The security-engineer fragment is authored at H2 and below — it does not introduce a `# Security Domain` H1 (the coordinator generates that when it assembles `docs/security/CLAUDE.md`)
- [ ] PASS: The skill does NOT write `docs/security/CLAUDE.md` directly — that file is the coordinator's to assemble from `_sections/`
...
- [ ] PASS: Output does not claim it overwrote or replaced `docs/security/_sections/security-engineer.md` — the language reflects merge, not replacement
